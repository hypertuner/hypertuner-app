  ...
  useEffect(() => {
    const handleMessage = ({ data }) => {
      ...
      if (jsonData.type === 'progress-data') {
        ...
        const displayableGraphs = Object.keys(jsonData.processStatus).filter(
          e => {
            return (
              jsonData.processStatus[e] === 'finished' ||
              jsonData.processStatus[e] === 'running'
            )
          }
        )
        setGraphDataMap(
          displayableGraphs.reduce((p, c) => {
            p[c] = null
            return p
          }, {})
        )
